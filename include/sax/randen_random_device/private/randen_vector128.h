...
static inline int128 load ( const uint64_t * __restrict__ lanes, const int block ) {
    const uint64_t * __restrict__ from = lanes + block * LANES;
    return _mm_load_si128 ( ( const int128 * ) from );
}

static inline void store ( const int128 v, uint64_t * __restrict__ lanes, const int block ) {
    uint64_t * __restrict__ to = lanes + block * LANES;
    _mm_store_si128 ( ( int128 * ) to, v );
}

static inline int128 aes ( const int128 state, const int128 round_key ) { return _mm_aesenc_si128 ( state, round_key ); }
...
static inline int128 load ( const uint64_t * __restrict__ lanes, const int block ) {
    const uint8_t * __restrict__ from = ( const uint8_t * ) ( lanes + block * LANES );
...
static inline int128 aes ( const int128 state, const int128 round_key ) { return vaesmcq_u8 ( vaeseq_u8 ( state, round_key ) ); }
